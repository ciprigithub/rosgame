...
void Player::process_laser_info(const sensor_msgs::msg::LaserScan::SharedPtr msg)
{
    ...
    // Parámetros de posición (solo si está activa la habilidad de piloto automático).
    // Solo enviamos el msg una vez.
    /*if (autopilot_enabled and enviar_posicion)
    {
        double pos_x = std::get<2>(result);
        double pos_y = std::get<3>(result);
        publish_pos(pos_x,pos_y);
        enviar_posicion = false;   
    }*/
}
...
{
    double linear, angular, goal_x, goal_y;
    ...
    {   
        ...
        if ( not objetivo_fijado )
        {
            ...
            // a. El robot tiene un buen nivel de batería, no posee el arma y conoce la posición de algún bloque de habilidades.
            if ( battery>50 and not(hammer_enabled) and not(skills_pos_array.empty()) )
            {
                ...
                // Si el jugador no tiene la habilidad de piloto automático se sigue el método tradicional.
                if ( autopilot_enabled )
                {   goal_x = objetivo_x;
                    goal_y = objetivo_y;
                    publish_pos(goal_x,goal_y);
                }
            ...
            // b. El robot tiene batería, posee el arma y conoce la posición de algún oponente.
            else if ( battery > 30 and hammer_enabled and not(players_pos_array.empty()) )
            {
                // Se calcula la distancia a cada oponente y se va al más cercano.
                float distance_min = std::numeric_limits<float>::max();
                for (size_t k = 0; k < players_pos_array.size(); ++k)
                {
                    float delta_x = pos_x - players_pos_array[k][0];
                    float delta_y = pos_y - players_pos_array[k][1];
                    float distance = sqrt(delta_x*delta_x + delta_y*delta_y);

                    if ( (distance < distance_min) or k==0 )
                    {
                        distance_min = distance;
                        objetivo_x = players_pos_array[k][0];
                        objetivo_y = players_pos_array[k][1];
                    }
                }
                // Si el jugador no tiene la habilidad de piloto automático se sigue el método tradicional.
                if ( autopilot_enabled )
                ...
                objetivo_fijado = true;
                tipo_objetivo = "Oponent";
                RCLCPP_INFO(this->get_logger(), "¡Nuevo objetivo! Oponente en la ubicación [X]='%f', [Y]='%f'.", objetivo_x, objetivo_y);
            }

            // c. El nivel de batería del robot está por debajo de un umbral y se conoce la posición de alguna plataforma de recarga.
            //    El umbral depende de si el jugador está persiguiendo a un jugador o si simplemente está navegando.
            else if ( !chargers_pos_array.empty() and ( (battery<50 and tipo_objetivo!="Oponent") or (battery<30 and tipo_objetivo=="Oponent") ) )
            {
            ...
            // d. En cualquier otro caso, avanzar en línea recta.
            else
            {
                linear = 1; angular = 0;
            }
        }
        // PASO 2. IR A POR EL OBJETIVO (SIN PILOTO AUTOMÁTICO)
        else if (objetivo_fijado and not(autopilot_enabled))
        {
            ...
            // El umbral cuando está siguiendo a un oponente es mayor porque no es necesario ser tan preciso.
            float umbral_angular = 0;
            if (tipo_objetivo=="Oponent")
            {   umbral_angular = 0.25;  }
            else
            {   umbral_angular = 0.15;  }
            if ( (diferencia_angular > umbral_angular) or (diferencia_angular < -1*umbral_angular) )
            {   
            ...
            {  
                // Si el destino está fijado en un oponente, debe ir más rápido para alcanzarlo.
                if (tipo_objetivo=="Oponent")
                {   angular = 0; linear = 2;    }
                else
                {   angular = 0; linear = 1;    }
            ...
            else if (tipo_objetivo=="Oponent" and distance<0.6)
            {
                RCLCPP_INFO(this->get_logger(), "¡Posición alcanzada! Buscando un nuevo objetivo...");
                objetivo_fijado = false;
                tipo_objetivo = "";
            }
        }
        // PASO 2. IR A POR EL OBJETIVO (USANDO PILOTO AUTOMÁTICO)
        else
        {
            // Se calcula la distancia entre el robot y el objetivo.
            float distance = sqrt( pow(objetivo_y - pos_y, 2) + pow(objetivo_x - pos_x, 2) );
            // Cuando la distancia sea inferior al umbral se puede cambiar de objetivo.
            if (distance < 0.5)
            {
                RCLCPP_INFO(this->get_logger(), "¡Posición alcanzada! Buscando un nuevo objetivo...");
                objetivo_fijado = false;
    ...
    return std::make_tuple(linear, angular, goal_x, goal_y);
}
...
void Player::publish_pos(double pos_x, double pos_y)
{
    rosgame_msgs::msg::RosgamePoint cmd_pos;
    
    cmd_pos.pos.x = pos_x;
    cmd_pos.pos.y = pos_y;
    cmd_pos.code = code;

    pub2_->publish(cmd_pos);   
}